...
class Device:
    ...
    def connect(self, dev):
        ...
        self.ser = serial.Serial(dev, 460800, timeout=1)  # original speed was 115200 (two)
        if not self.requestInfo(3):
            self.disconnect()
            return False
        if self.testmode:
            print("Connection to ", self.ser.name, " was successfull, but the device is running the hardware test firmware,"
                                                   "which cannot be used for anything but testing. Please flash the proper inkkeys firmware to use it.")
            return False
...
# Center jog wheel and title label (the title get's small -0.5 nudge for rounding to prefer a top alignment)
            position2 = None
        elif function < 6:
