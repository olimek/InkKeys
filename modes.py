#  here, the logic of the different modes are defined.
...
# At the end you have to return an interval in seconds before the function is to be called again - otherwise it is not called a second time
# - animate
...
# To avoid multiple screen refreshs, the modules usually do not clean-up the display when being deactivvated. Instead,
#  each module is supposed to set at least the area corresponding to each button (even if it needs to be set to white if unused).
...
#  obswebsocket import obsws, requests, events # Control OBS. This requires the websocket plugin in OBS (https://github.com/Palakis/obs-websocket)
# and the Python library obs-websocket-py (pip3 install obs-websocket-py, https://github.com/Elektordi/obs-websocket-py)
...
class ModeProgrammingVSC:
    ...
    def activate(self, device):
        device.sendTextFor("title", "VSC", inverted=True)  # Title
...
class ModeProgrammingSTM:
    lightState = None  # Current state of the lights in my office. (Keeping track to know when to update the screen)
    demoActive = False  # We have a demo button and this keeps track whether the demo mode is active, so we know when to update the screen
    jogFunction = ""

    def activate(self, device):
        device.sendTextFor("title", "STMCube", inverted=True)  # Title
...
class ModeDefault:
    ...
    def activate(self, device):
        ...
        device.assignKey(
            KeyCode.SW3_PRESS,
            [event(DeviceCode.CONSUMER, ConsumerKeycode.MEDIA_PREV, ActionCode.PRESS)],
        )
        device.assignKey(
            KeyCode.SW3_RELEASE,
        ...
        device.assignKey(
            KeyCode.SW6_PRESS,
            [event(DeviceCode.CONSUMER, ConsumerKeycode.MEDIA_STOP, ActionCode.PRESS)],
        )
        device.assignKey(
            KeyCode.SW6_RELEASE,
        ...
        device.assignKey(
            KeyCode.SW7_PRESS,
            [event(DeviceCode.CONSUMER, ConsumerKeycode.MEDIA_NEXT, ActionCode.PRESS)],
        )
        device.assignKey(
            KeyCode.SW7_RELEASE,
        ...
        device.assignKey(
            ...
            [
                event(
                    DeviceCode.CONSUMER,
                    ConsumerKeycode.CONSUMER_EMAIL_READER,
                    ActionCode.PRESS,
                )
            ],
        )
        ...
        device.assignKey(
            ...
            [
                event(
                    DeviceCode.CONSUMER,
                    ConsumerKeycode.CONSUMER_CALCULATOR,
                    ActionCode.PRESS,
                )
            ],
        )
...
class ModeBlender:
    def activate(self, device):
        ...
        device.assignKey(
            KeyCode.SW2_PRESS,
            [event(DeviceCode.KEYBOARD, KeyboardKeycode.KEYPAD_0, ActionCode.PRESS)],
        )  # Set view to camera
        device.assignKey(
            KeyCode.SW2_RELEASE,
    ...
    def animate(self, device):
        device.fadeLeds()  # No LED animation is used in this mode,
        # but we call "fadeLeds" anyway to fade colors that have been set in another mode before switching
...
class ModeGimp:
    ...
    def activate(self, device):
        ...
        device.assignKey(
            KeyCode.SW2_PRESS,
            [
                event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_LEFT_ALT, ActionCode.PRESS),
                event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_B),
                event(
                    DeviceCode.KEYBOARD,
                    KeyboardKeycode.KEY_LEFT_ALT,
                    ActionCode.RELEASE,
                ),
                event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_Z),
            ],
        ...
        device.assignKey(
            ...
            [
                ...
                event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_I),
            ],
        ...
        device.assignKey(
            ...
            [
                ...
                event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_L),
            ],
        )  # Canvas size (this shortcut appears to be language
        device.assignKey(KeyCode.SW4_RELEASE, [])
        ...
        )  # Resize (this shortcut appears to be language
        device.assignKey(KeyCode.SW5_RELEASE, [])
        ...
        def toggleJogFunction(update=True):
            if self.jogFunction == "size":  # Tool opacity in GIMP
                ...
                device.assignKey(
                    KeyCode.JOG_CW,
                    [event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_LEFT_BRACE)],
                )
                device.assignKey(
                    KeyCode.JOG_CCW,
                    [event(DeviceCode.KEYBOARD, KeyboardKeycode.KEY_RIGHT_BRACE)],
                )
    ...
    def activate(self, device):
        ...
        def toggleJogFunction(update=True):
            if self.jogFunction == "wheel":
                ...
                device.assignKey(
                    KeyCode.JOG_CW,
                    [event(DeviceCode.CONSUMER, ConsumerKeycode.MEDIA_VOL_UP)],
                )
                device.assignKey(
                    KeyCode.JOG_CCW,
                    [event(DeviceCode.CONSUMER, ConsumerKeycode.MEDIA_VOL_DOWN)],
                )
    ...
    def poll(self, device):
        if not self.demoActive:
            ...
            if light != self.lightState:
                ...
                self.showLightState(device)
        return 10  # Since we only retrieve the current state from the Mqtt class,
        # the 10 seconds do not really control how often the value is querries but how often we react to it.
        #  For the CO2 warning, this is a not too intrusive blue flash every 10 seconds and
        #  a 10 second delay to update the light state if it has been switched from somewhere else (rare) seems reasonable, too.

    # Called to update the icon of button 4, showing the state of the office light
    #  (as if I couldn't see it in the real room, but it is a nice touch to update the display accordingly)
...
class ModeOBS:
    ...
    # State of sources within scenes. "items" is an array of scene/item combinations
    #  to keep track of items that need to be switched on multiple scenes simultaneously,
    #  so you can mute all mics in all scenes and switch scenes without an unpleasant surprise.
    #  The current state is tracked in this object ("current")
    states = [
        {
            "items": [
                ("Moderation", "Phone"),
    ...
    # Updates the buttons associated with scenes. Unless "init" is set to true,
    #  it only updates changed parts of the display and returns True if anything has changed so that the calling function should call updateDisplay()
    ...
    # Updates the buttons associated with states. Unless "init" is set to true,
    #  it only updates changed parts of the display and returns True if anything has changed so that the calling function should call updateDisplay()
    ...
    def updateLED(self, device):
        if self.currentScene == "Video-Mute" or self.states[2]["current"] is False:
    ...
    def activate(self, device):
        ...
        # ## Buttons 7 to 9 toogle the visibility of items,
        #  some of which are present in multiple scenes (Mics, Picture-In-Picture cam, Video stream from phone) ###
