...
import serial  # import SerialException   #Serial functions
...
SERIALPORT = None  # None = Auto-detect, "COM1" (Windows)
VID = 0x2341  # USB Vendor ID for a Pro Micro
PID = 0x8036  # USB Product ID for a Pro Micro
DEBUG = True  # More output on the command line

# python -m PyQt5.uic.pyuic -x C:\Users\molichwer\Desktop\MOL\untitled.ui -o C:\Users\molichwer\Desktop\MOL\aha.py

# from mqtt import InkkeysMqtt #A small class to encapsule MQTT specific functions.
# You will need to adapt this to your needs if you want to use this.

print("Ollie macro-keyboard")
...
modes = [  # {"mode": ModeOBS(), "process": "obs"}, \
    {"mode": ModeBlender(), "activeWindow": re.compile("^Blender")},
    {"mode": ModeProgrammingSTM(), "activeWindow": re.compile("STM32CubeIDE")},
...
def work():
    ...
    mode = ModeDefault().activate(device)  # Current mode of the device (i.e. key mappings for specific process).
    # device.updateDisplay(fullRefresh=Tru
    try:
        while True:  # Now we are in our main, infinite loop -------------------
            now = time.time()  # Time of this iteration

            if now - lastModeCheck > 0.5:  # Check active window and decide which mode to use. This can be done more regularly,
                window = getActiveWindow()  # Get the currently active window
                if window is not None:  # Sometime getting the active window fails, then ignore it. (Some window managers allow having no window in focus)
                    activeWindow = window
...
# If it succeeds, it will enter the main working loop forever. It will only return if an error occurs and return
# True to report that it was working with the correct device.
def tryUsingPort(port):
    t1 = Thread(target=work)
    t2 = Thread(target=workProcesses)
...
try:
    while True:
        if SERIALPORT is not None:  # Explicit port has been defined
            ...
            for port in serial.tools.list_ports.comports():  # Iterate over all serial ports
                ...
                if tryUsingPort(port.device):  # Try connecting to this device
                    break  # Connection was successful and inkkeys was found. If we reach this point,
        # we do not need to search on another port. We got disconnected or some other kind of error,
        #  so skip the rest of the port list and start over.
